...
import { prepareNumber } from '../../Utils/Helpers'
...
const ListPage = () => {
  ...
  const prepareData = (item: any): TCustomType => {
    const preparedItem = { ...item }
    preparedItem.price = prepareNumber(item.price)
    if (item.width) {
      preparedItem.width = prepareNumber(item.width.toString())
    }
    return preparedItem
  }
  ...
  const addItem = async (item: TCustomFormType) => {
    ...
    const response = await api.post(customOptions.apiPath, preparedItem)
    if (!response) return
    const newList = [...list]
  ...
  const editItem = async (item: TCustomFormType) => {
    ...
    const response = await api.put(
      `${customOptions.apiPath}/${preparedItem._id}`,
      preparedItem
    )
    if (!response) return
